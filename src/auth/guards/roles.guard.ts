...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const requiredAdminRoles = this.reflector.getAllAndOverride<
      AdminRole[] | undefined
    ...
    const req = context
      .switchToHttp()
      .getRequest<Request & { user?: JwtPayload | AdminUser }>();
    const user = req.user;
    ...
    if (
      ...
      ('role' in user && Object.values(AdminRole).includes(maybeAdmin.role))
    ) {
      ...
      if (requiredAdminRoles && requiredAdminRoles.length > 0) {
        if (!requiredAdminRoles.includes(adminRole)) {
          throw new ForbiddenException(
            `Ushbu amal uchun ruxsat berilmagan. Kerakli rol: ${requiredAdminRoles.join(', ')}`,
          );
        }
        return true;
      }
    ...
    if (requiredRoles && requiredRoles.length > 0) {
      if (!userRoles.some((r) => requiredRoles.includes(r))) {
        throw new ForbiddenException(
          `Bu amal uchun rollar kerak: ${requiredRoles.join(', ')}`,
        );
      }
    }
