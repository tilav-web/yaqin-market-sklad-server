...
export class RiskFlags1786309154860 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "risk_flags_severity_enum" AS ENUM('low', 'medium', 'high')`,
    );
    await queryRunner.query(
      `CREATE TYPE "risk_flags_status_enum" AS ENUM('open', 'confirmed', 'dismissed')`,
    );
    await queryRunner.query(
      `CREATE TYPE "risk_flags_subjecttype_enum" AS ENUM('user', 'shop', 'order', 'device')`,
    );
    ...
    await queryRunner.query(`
        ...
        `);
    await queryRunner.query(
      `CREATE INDEX "IDX_risk_flags_status_severity_lastSeen" ON "risk_flags" ("status", "severity", "lastSeenAt")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_risk_flags_subject" ON "risk_flags" ("subjectType", "subjectId")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_risk_flags_orderId" ON "risk_flags" ("orderId")`,
    );
    ...
    await queryRunner.query(`
        ...
        `);
    await queryRunner.query(
      `CREATE INDEX "IDX_device_accounts_userId" ON "device_accounts" ("userId")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX "IDX_risk_flags_status_severity_lastSeen"`,
    );
