...
export class OrderLocationEvidence1786303073706 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "dispatchedEvidence" jsonb`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "deliveredEvidence" jsonb`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "deliveredByUserId" uuid`,
    );

    await queryRunner.query(
      `ALTER TABLE "user_addresses" ADD "pinEvidence" jsonb`,
    );
    await queryRunner.query(
      `ALTER TABLE "user_addresses" ADD "pinSetCount" integer NOT NULL DEFAULT 0`,
    );

    await queryRunner.query(`ALTER TABLE "shops" ADD "pinEvidence" jsonb`);
    await queryRunner.query(
      `ALTER TABLE "shops" ADD "relocatedAt" TIMESTAMP WITH TIME ZONE`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user_addresses" DROP COLUMN "pinSetCount"`,
    );
    await queryRunner.query(
      `ALTER TABLE "user_addresses" DROP COLUMN "pinEvidence"`,
    );

    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "deliveredByUserId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "deliveredEvidence"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "dispatchedEvidence"`,
    );
